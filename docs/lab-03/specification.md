...
| FR-18 | IT Staff can set IT Priority, which initially copies Requested Priority, and can perform only the approved Ticket status transitions. |
...
| Claim, assign/reassign, IT Priority, status | No | Yes | No mutation |
...
Administrator and IT Staff responsibilities remain conceptually separate. An active Administrator is a valid Ticket owner or assignment target because the data contract requires that owner domain, but that fact does not automatically grant IT Staff Queue or Ticket mutation permissions. If a later requirement needs Administrators to work Tickets, it must be added to this matrix and reviewed before implementation.
...
| BR-07 | Sessions use an opaque random cookie value, expire after the documented lifetime, and are invalidated on logout. Expired or revoked sessions cannot access protected resources. |
...
| BR-14 | Requested Priority remains the value submitted by the Requester. IT Priority is initialized to the same value and may be changed only by an authorized IT Staff operation. |
...
| ID | Rule |
| --- | --- |
| BR-17 | Only IT Staff can change IT Priority or Ticket status. Requesters cannot formally resolve or close Tickets. |
...
| BR-26 | An Administrator cannot deactivate their own account. The system cannot deactivate or remove the last active Administrator. User deletion is not available; deactivation is used instead. |
...
4. Generate a cryptographically random one-time initial password for migrated Users, store only its hash, set `mustChangePassword = true`, and provide the password only through a local operator handoff. No migration password is committed to the repository or printed in application logs.
...
| AC-19 | Given a Ticket, when IT Staff changes IT Priority, then the initial value equals Requested Priority and later changes are restricted to authorized IT Staff operations. |
...
| AC-21 | Given a Ticket with comments and notes, when each role reads it, then Public Comments and Internal Notes follow the visibility matrix and both remain append-only. |
...
| AC-24 | Given an existing User, when an Administrator edits name, email, role, or activation state, then valid changes persist and duplicate emails or invalid roles are rejected safely. |
...
- Keep Administrator access conceptually separate from IT Staff operations. Administrators can read the communication visibility required by the handout but do not receive staff mutation permissions through this contract.

...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{29mm}@{\hspace{2mm}}>{\raggedright\arraybackslash}p{0.34\linewidth}@{\hspace{2mm}}>{\raggedright\arraybackslash}p{0.42\linewidth}@{}}
  ...
  \href{https://github.com/ovenmakemeheat/toktickit/pull/47}{\#47 category list} \textit{(Open)} & \promptquote{All Be Good for Me} & \promptquote{Thanks!} The pull request remains open for human workflow completion. \\
  \bottomrule
\end{longtable}
}
...
The following captures provide human-checked evidence for the board state, merge history, repository structure, and automated test output. GitHub captures were taken with browser-use CLI from the \texttt{lab1-staging} view.

\begin{figure}[H]
  \centering
  \workflowimage{docs/lab-01/evidence/board.png}{Capture the completed Lab 1 project board.}
  \caption{Lab 1 project board: six workflow columns, four parent issues, and visible sub-issue progress.}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Terminal evidence: the client suite passes 3 tests and the server suite passes 6 tests.}
\end{figure}
